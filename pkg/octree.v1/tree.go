...
func (t *Tree) add(b gfx.Boundable, bb lmath.Rect3, target *Node) {
	...
	for oct = 0; oct < 9; oct++ {
		...
		for objIndex, o = range objs {
			...
			if oct != 9 && fittingChild == oct {
				...
				fitsParent := bb.In(n.parent.bounds)
				if fitsParent {
					// It still fits in the parent node; add it there.
					addTarget = n.parent
				}
			}
			goto removal
...
func NewTree(k int, b lmath.Rect3) *Tree {
	t := &Tree{
		numNodes:     1,
		splitFactor:  k,
...
//  NewTree(100, lmath.Rect3Zero)
func New() *Tree {
	return NewTree(100, lmath.Rect3Zero)
}
