...
func (f *TruetypeFont) Index(r rune) (idx FontIndex, ok bool) {
	idx = FontIndex(f.Font.Index(r))
	ok = idx != 0
	return
}
...
func (f *TruetypeFont) expandContour(glyph *QuadGlyph, points []truetype.Point) {
	...
	// mid finds the midpoint of a and b.
	...
	// Check for proper expression of contour points.
	mod := len(glyph.Points[startIndex:]) % 2
	if mod != 0 {
		panic(fmt.Sprintf("Failed to express contour points: len(contour) % 2 == %d", mod))
	}

	// TODO(slimsag): remove this shift and place it into the above generation
	// code instead.
	postStart := glyph.Points[startIndex:]
	oldEnd := postStart[len(postStart)-1]
	for j := len(postStart)-1; j > 0; j-- {
		postStart[j] = postStart[j-1]
	}
	postStart[0] = oldEnd

	// Append the contour index.
