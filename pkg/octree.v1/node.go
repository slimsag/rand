...
func (n *Node) Bounds() lmath.Rect3 {
	n.access.RLock()
	b := n.bounds
	n.access.RUnlock()
	return b
...
func (n *Node) childBounds(i ChildIndex) lmath.Rect3 {
	c := n.bounds
...
func (n *Node) childFits(b lmath.Rect3) ChildIndex {
	nb := n.bounds
	center := nb.Center()
...
// node.
func (n *Node) expand(r lmath.Rect3) *Node {
	nb := n.bounds
	if nb.Empty() {
		...
		s /= 2
		s *= 32
		startSize := lmath.Vec3{s, s, s}
		n.bounds = lmath.Rect3{
...
// r can be placed.
func (n *Node) findPlace(r lmath.Rect3) (*Node, ChildIndex) {
	if !r.In(n.bounds) {
		return nil, -1
	}
