...
// ByDist sorts a list of spatial objects based on their distance away from
...
type ByDist struct {
	// The list of spatial objects to sort.
	Objects []Spatial
...
func (b ByDist) Less(ii, jj int) bool {
	i := b.Objects[ii].Bounds()
	j := b.Objects[jj].Bounds()

	// Calculate the distance from each object to the target position.
	iDist := i.Closest(b.Target).Sub(b.Target).LengthSq()
	jDist := j.Closest(b.Target).Sub(b.Target).LengthSq()
